...
import { Moon, Bell, Mic, Smartphone, Volume2, X } from "lucide-react";
...
export default function SettingsPanel({ settings, setSettings, onClose }) {
  try {
    ...
    return (
      <div
        className={`h-full ${bgColor} p-6 overflow-y-auto`}
        data-name="settings-panel"
      >
        <div className="max-w-2xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            <button
              onClick={onClose}
              className={`w-12 h-12 rounded-full ${cardBg} flex items-center justify-center`}
            >
            ...
            <div className={`p-4 rounded-xl ${cardBg}`}>
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-3">
                  <Volume2 className={textColor} size={24} />
  ...
  return (
    <div className={`p-4 rounded-xl ${cardBg} flex items-center justify-between`}>
      ...
      <label className="relative inline-flex items-center cursor-pointer">
        ...
        />
        <div className="w-14 h-8 bg-gray-300 rounded-full peer-checked:bg-[var(--primary-color)] transition-colors">
          <div
            className={`absolute top-1 left-1 bg-white w-6 h-6 rounded-full transition-transform ${checked ? "translate-x-6" : ""
              }`}
          ></div>
        </div>
      </label>
